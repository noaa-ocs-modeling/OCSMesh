...
def reader(path):
    ...
    with open(pathlib.Path(path), 'r') as f:
        ...
        if '+init=' in line:
            line = line.split('+init=')
            grd['crs'] = line[1].split()[0]
            grd['description'] = line[0]
        else:
            grd['crs'] = None
            grd['description'] = line
...
def graph(grd):
    description = grd['description'].strip('\n')
    f = f"{description}\n"
    if "crs" in grd:
        if grd["crs"] is not None:
            if len(grd['description']) > 0:
                f += " "
            f += f"+init={grd['crs']}"
