...
class Raster:
    ...
    # def __iter__(self):
    #     for window in self._iter_windows(self.overlap):
    #         bounds = array_bounds(
    #             window.height,
    #             window.width,
    #             self._get_window_transform(window)
    #         )
    #         yield window, bounds

    def x(self, window=None):
        ...
        if window is not None:
            ...
            width = self.shape[0]
        x0, y0, x1, y1 = self._get_array_bounds(window)
        return np.linspace(x0, x1, width)

    def y(self, window=None):
        window = windows.Window(0, 0, self._src.shape[1], self._src.shape[0]) \
            if window is None else window
        if window is not None:
            assert isinstance(window, windows.Window)
    ...
    def get_multipolygon(
        ...
        if driver == 'rasterio':
            ...
            with rasterio.open(tmpfile.name, 'w', **self._src.meta) as dst:
                for window in self._iter_windows():
                    data = self.values(window=window, masked=True)
                    if zmin is not None:
        ...
        if driver == 'matplotlib':
            overlap = self.overlap if overlap is None else 1
            for window in self._iter_windows(overlap=overlap):
                x, y, z = self._get_window_data(window)
    ...
    def fill_nodata(self):
        ...
        with rasterio.open(tmpfile.name, 'w', **self._src.meta.copy()) as dst:
            for window in self._iter_windows(self.chunk_size):
                dst.write(
    ...
    def _get_window_data(self, window, masked=True):
        x0, y0, x1, y1 = self._get_array_bounds(window)
    ...
    def _get_array_bounds(self, window):
        return array_bounds(
            window.height,
            window.width,
            self._get_window_transform(window))

    def _get_window_transform(self, window):
        if window is None:
            return
        return windows.transform(window, self.transform)

    def _iter_windows(self, chunk_size=None, overlap=0):
    ...
    def _src_crs(self, src_crs):
        if src_crs is None:  # return if CRS in file || raise if no CRS
            ...
            raise NotImplementedError('Raster._src_crs.setter')
    ...
    def _src(self):
        ...
        #         polygon_collection = []
        #         for window in self._iter_windows(overlap=overlap):
        #             mask = self._src.read(masked=True, window=window).mask
        #             x0, y0, x1, y1 = self._get_array_bounds(window)
        #             if not mask.any():
        #                 polygon_collection.append(box(x0, y0, x1, y1))
        #             else:
        #                 x = np.linspace(x0, x1, window.width)
        #                 y = np.linspace(y1, y0, window.height)
        #                 new_mask = np.full(mask.shape, 0)
        #                 new_mask[np.where(mask)] = -1
        #                 new_mask[np.where(~mask)] = 1
        #                 ax = plt.contourf(
        #                     x, y, new_mask[0, :, :], levels=[0, 1])
        #                 plt.close(plt.gcf())
        ...
        # # contour based
        # else:
        #     zmin = np.min(self.values) if self.zmin is None else self.zmin
        #     zmax = np.max(self.values) if self.zmax is None else self.zmax
        #     polygon_collection = []
        #     for window in self._iter_windows(overlap=overlap):
        #         data = self._src.read(masked=True, window=window)
        #         x0, y0, x1, y1 = self._get_array_bounds(window)
        #     ax = plt.contourf(
