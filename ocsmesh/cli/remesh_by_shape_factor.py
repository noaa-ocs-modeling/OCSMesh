    ...
    def run(self, args):
        ...
        if shape_path or refine_cutoff is not None:
            ...
            if shape_path:
                ...
                gdf_diff = gpd.overlay(gdf_mesh_poly, gdf_shape, how="difference")
                diff_polys = []
                ...
                if refine_upstream:
                    ...
                    if upstream_polys:
                        gdf_upstream = gpd.GeoDataFrame(
                            geometry=gpd.GeoSeries(upstream_polys), crs=gdf_diff.crs
                        )
            ...
            utils.clip_mesh_by_shape(
                init_jig, refine_polys, fit_inside=True, inverse=True, in_place=True
            )
        ...
        for level, expansion_rate, target_size in contour_defns:
            ...
            hfun_refine.add_feature(
                refine_ctr, expansion_rate, target_size, nprocs=nprocs
            )
        ...
        for lower_bound, expansion_rate, target_size in patch_defns:
            ...
            hfun_refine.add_patch(refine_mp, expansion_rate, target_size, nprocs)
        ...
        final_hfun_jig = utils.merge_msh_t(
            initial_hfun_jig, refine_hfun_jig, out_crs=ref_crs, drop_by_bbox=False
        )

        if not (geom_jig.crs == ref_crs and (init_jig and init_jig.crs == ref_crs)):
            raise ValueError("CRS for geometry, hfun and init mesh is not the same")
        ...
        jigsawpy.lib.jigsaw(
            opts, geom_jig, remesh_jig, init=init_jig, hfun=final_hfun_jig
        )

        utils.finalize_mesh(remesh_jig, sieve)

        # Interpolate from inpu mesh and DEM if any
        utils.interpolate_euclidean_mesh_to_euclidean_mesh(mesh.msh_t, remesh_jig)
