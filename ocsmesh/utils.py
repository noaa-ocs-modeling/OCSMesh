...
def cleanup_isolates(mesh):
    ...
    vert2_idxs = np.where(np.isin(node_indexes, used_indexes, assume_unique=True))[0]
...
def put_edge2(mesh):
    ...
    mesh.edge2 = np.array([(edge, 0) for edge in tri.edges], dtype=jigsaw_msh_t.EDGE2_t)
...
def get_boundary_segments(mesh):
    ...
    vert_map = {orig: new for new, orig in enumerate(boundary_verts)}
    new_boundary_edges = np.array(
        [vert_map[v] for v in boundary_edges.ravel()]
    ).reshape(boundary_edges.shape)

    graph = sparse.lil_matrix((len(boundary_verts), len(boundary_verts)))
    ...
    for i in range(n_components):
        conn_mask = np.any(np.isin(new_boundary_edges, np.nonzero(labels == i)), axis=1)
        ...
        if not this_segment.is_simple:
            ...
            if not test_polys:
                raise ValueError("Mesh boundary crosses itself! Folded element(s)!")
...
def _sieve_by_mask(mesh, sieve_mask):
    ...
    # update tria3
    mesh.tria3 = np.array(
        [(tuple(indices), tria3_id_tag[i]) for i, indices in enumerate(tria3_index)],
        dtype=jigsaw_msh_t.TRIA3_t,
    )
...
def remesh_small_elements(opts, geom, mesh, hfun):
    ...
    for coef in coeffs:
        ...
        tiny_verts = np.unique(
            fixed_mesh.tria3["index"][tria_areas < tiny_sz, :].ravel()
        )
        if len(tiny_verts) == 0:
            break
        ...
        jigsawpy.lib.jigsaw(opts, geom, fixed_mesh, init=mesh_clip, hfun=hfun)
...
def index_ring_collection(mesh):
    ...
    for i, j in idxs:
        boundary_edges.append(
            (int(tri.triangles[i, j]), int(tri.triangles[i, (j + 1) % 3]))
        )
    ...
    _id = 0
    idx_ring_coll = {}
    idx_ring_coll[_id] = {"exterior": np.asarray(exterior), "interiors": []}
...
def get_multipolygon_from_pathplot(ax):
    ...
    if len(linear_ring_collection) > 1:
        ...
        while len(linear_ring_collection) > 0:
            inner_rings = []
            for i, linear_ring in reversed(list(enumerate(linear_ring_collection))):
            ...
            if len(linear_ring_collection) > 0:
                areas = [
                    Polygon(linear_ring).area for linear_ring in linear_ring_collection
                ]
                idx = np.where(areas == np.max(areas))[0][0]
                outer_ring = linear_ring_collection.pop(idx)
                path = Path(np.asarray(outer_ring.coords), closed=True)
...
def get_surrounding_elem_verts(mesh, in_vert):
    ...
    # are considered too
    mark_tria = np.any((np.isin(tria.ravel(), in_vert).reshape(tria.shape)), 1)
    mark_quad = np.any((np.isin(quad.ravel(), in_vert).reshape(quad.shape)), 1)
    mark_hexa = np.any((np.isin(hexa.ravel(), in_vert).reshape(hexa.shape)), 1)

    conn_verts = np.unique(
        np.concatenate(
            (
...
def get_lone_element_verts(mesh):
    ...
    unq_verts, counts = np.unique(
        np.concatenate((tria.ravel(), quad.ravel(), hexa.ravel())), return_counts=True
    )
    ...
    mark_tria = np.all((np.isin(tria.ravel(), once_verts).reshape(tria.shape)), 1)
    mark_quad = np.all((np.isin(quad.ravel(), once_verts).reshape(quad.shape)), 1)
    mark_hexa = np.all((np.isin(hexa.ravel(), once_verts).reshape(hexa.shape)), 1)

    lone_elem_verts = np.unique(
        np.concatenate(
            (
                tria[mark_tria, :].ravel(),
                quad[mark_quad, :].ravel(),
...
def get_verts_in_shape(
    ...
    if from_box:
        ...
        in_box_idx = reduce(
            np.intersect1d, (in_box_idx_1, in_box_idx_2, in_box_idx_3, in_box_idx_4)
        )
        return in_box_idx

    pt_series = gpd.GeoSeries(
        gpd.points_from_xy(mesh.vert2["coord"][:, 0], mesh.vert2["coord"][:, 1])
    )
    shp_series = gpd.GeoSeries(shape)

    in_shp_idx = pt_series.sindex.query_bulk(shp_series, predicate="intersects")
...
def get_cross_edges(
    ...
    gdf_edg = gpd.GeoDataFrame(geometry=gpd.GeoSeries(linemerge(edge_coords)))
    ...
    cut_coords = [
        list(cooseq) for cooseq in gdf_x.geometry.apply(lambda i: i.coords).values
    ]
    ...
    cut_edge_idx = np.array(
        [coord_dict[tuple(coo)] for coo in cut_edges.reshape(-1, 2)]
    ).reshape(cut_edges.shape[:2])
...
def clip_mesh_by_shape(
    ...
    if not inverse or use_box_only:
        ...
        mesh = clip_mesh_by_vertex(mesh, in_box_idx, not fit_inside, inverse, in_place)
    ...
    mesh = clip_mesh_by_vertex(mesh, in_shp_idx, not fit_inside, inverse, in_place)
...
def remove_mesh_by_edge(
    mesh: jigsaw_msh_t, edges: Sequence[Tuple[int, int]], in_place: bool = False
) -> jigsaw_msh_t:
...
def clip_mesh_by_vertex(
    ...
    if mesh.mshID == "euclidean-mesh" and mesh.ndims == 2:
        ...
        elm_dict = {key: getattr(mesh, key)["index"] for key in mesh_types}
    ...
    msg = f"Not implemented for" f" mshID={mesh.mshID} and dim={mesh.ndims}"
    raise NotImplementedError(msg)
...
def get_mesh_edges(mesh: jigsaw_msh_t, unique=True):
    ...
    for elm_type in [trias, quads, hexas]:
        if elm_type.shape[0]:
            edges = np.sort(
                np.stack((elm_type, np.roll(elm_type, shift=1, axis=1)), axis=2), axis=2
            )
...
def calculate_tria_areas(mesh):
    ...
    tria_coo = coord[
        np.sort(np.stack((trias, np.roll(trias, shift=1, axis=1)), axis=2), axis=2)
    ]
    tria_side_components = np.diff(tria_coo, axis=2).squeeze()
    tria_sides = np.sqrt(
        np.sum(np.power(np.abs(tria_side_components), 2), axis=2).squeeze()
    )
    ...
    tria_areas = np.sqrt(
        perimeter * (perimeter - a_side) * (perimeter - b_side) * (perimeter - c_side)
    ).squeeze()
    return tria_areas
...
def calculate_edge_lengths(mesh, transformer=None):
    ...
    if transformer is not None:
        coord = np.vstack(transformer.transform(coord[:, 0], coord[:, 1])).T
    ...
    edge_lens = np.sqrt(
        np.sum(np.power(np.abs(np.diff(edge_coords, axis=1)), 2), axis=2)
    ).squeeze()
...
def elements(mesh):
    ...
    elems = {elements_id[i]: indexes for i, indexes in enumerate(elems)}
    return elems
...
def interpolate_euclidean_grid_to_euclidean_mesh(
    src: jigsaw_msh_t, dst: jigsaw_msh_t, bbox=None, kx=3, ky=3, s=0
):
...
def tricontourf(
    mesh, ax=None, show=False, figsize=None, extend="both", colorbar=False, **kwargs
):
    if ax is None:
...
def triplot(
    mesh, axes=None, show=False, figsize=None, color="k", linewidth=0.07, **kwargs
):
    if axes is None:
        fig = plt.figure(figsize=figsize)
...
def reproject(mesh: jigsaw_msh_t, dst_crs: Union[str, CRS]):
    src_crs = mesh.crs
    ...
    mesh.vert2 = np.array(
        [
            ([x[i], y[i]], mesh.vert2["IDtag"][i])
            for i in range(len(mesh.vert2["IDtag"]))
        ],
        dtype=jigsaw_msh_t.VERT2_t,
    )
    mesh.crs = dst_crs
...
def msh_t_to_grd(msh: jigsaw_msh_t) -> Dict:
    ...
    elems = {i + 1: v + 1 for i, v in enumerate(msh.tria3["index"])}
    offset = len(elems)
    elems.update({offset + i + 1: v + 1 for i, v in enumerate(msh.quad4["index"])})

    return {"description": desc, "nodes": nodes, "elements": elems}
...
def msh_t_to_utm(msh):
    ...
    transformer = Transformer.from_crs(msh.crs, utm_crs, always_xy=True)
...
def estimate_bounds_utm(bounds, crs="EPSG:4326"):
    ...
    if in_crs.is_geographic:
        ...
        utm_crs = CRS(
            ...
            zone=f"{number}",
            south=(y0 + y1) / 2 < 0,
            ellps={"GRS 1980": "GRS80", "WGS 84": "WGS84"}[in_crs.ellipsoid.name],
        )
        return utm_crs
...
def get_polygon_channels(polygon, width, simplify=None, join_style=3):
    ...
    polys_gdf = gpd.GeoDataFrame(geometry=gpd.GeoSeries(polygon))

    if isinstance(simplify, (int, float)):
        polys_gdf = gpd.GeoDataFrame(
            geometry=polys_gdf.simplify(tolerance=simplify, preserve_topology=False)
        )
    ...
    if len(buffered_gdf) == 0:
        # All is channel!
        return polygon
    channels_gdf = gpd.overlay(polys_gdf, buffered_gdf, how="difference")
    ...
    channels_gdf = gpd.GeoDataFrame(
        geometry=gpd.GeoSeries(
            [
                p
                for i in channels_gdf.geometry
                for p in i.geoms
...
def merge_msh_t(
    *mesh_list,
    ...
    composite_mesh.tria3 = np.array(
        [(index, 0) for index in np.vstack(index)], dtype=jigsaw_msh_t.TRIA3_t
    )
    composite_mesh.value = np.array(np.vstack(value), dtype=jigsaw_msh_t.REALS_t)
