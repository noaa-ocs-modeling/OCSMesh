...
class HfunInputRaster:
    def __set__(self, obj, raster: Raster):
        ...
        with ExitStack() as stack:
            ...
            dst = stack.enter_context(obj.modifying_raster(use_src_meta=False, **meta))
...
class HfunRaster(BaseHfun, Raster):
    ...
    def msh_t(
        ...
        for win in iter_windows:
            ...
            utm_crs = utils.estimate_bounds_utm((x0, y0, x1, y1), self.crs)
            ...
            if utm_crs is not None:
                ...
                hfun.vert2 = np.empty(
                    win.width * win.height, dtype=jigsaw_msh_t.VERT2_t
                )
                hfun.vert2["coord"] = np.array(self.get_xy_memcache(win, utm_crs))
                _logger.info("Done building hfun.vert2...")

                # Build REALS_t: this one comes from hfun raster
                _logger.info("Building hfun.value...")
                hfun.value = np.array(
                    self.get_values(window=win, band=1)
                    .flatten()
                    .reshape((win.width * win.height, 1)),
                ...
                geom = PolygonGeom(
                    ops.transform(transformer.transform, Polygon(bbox)), utm_crs
                ).msh_t()
                ...
                hfun.xgrid = np.array(
                    np.array(self.get_x(window=win)), dtype=jigsaw_msh_t.REALS_t
                )
                hfun.ygrid = np.array(
                    np.flip(self.get_y(window=win)), dtype=jigsaw_msh_t.REALS_t
                )
                hfun.value = np.array(
            ...
            output_mesh.vert2 = np.append(
                output_mesh.vert2,
                np.array(list(window_mesh.vert2), dtype=jigsaw_msh_t.VERT2_t),
                axis=0,
            )
            if output_mesh.value.size:
                output_mesh.value = np.append(
                    output_mesh.value,
                    np.array(list(window_mesh.value), dtype=jigsaw_msh_t.REALS_t),
                    axis=0,
                )
            else:
                output_mesh.value = np.array(
                    list(window_mesh.value), dtype=jigsaw_msh_t.REALS_t
                )
        ...
        if utm_crs is not None:
            ...
            output_mesh.vert2["coord"] = np.vstack(
                transformer.transform(
                    output_mesh.vert2["coord"][:, 0], output_mesh.vert2["coord"][:, 1]
                )
            ).T
    ...
    def add_topo_bound_constraint(
        ...
        self._constraints.append(
            TopoConstConstraint(value, upper_bound, lower_bound, value_type, rate)
        )
    ...
    def add_topo_func_constraint(
        ...
        # TODO: Validate conflicting constraints, right now last one wins
        self._constraints.append(
            TopoFuncConstraint(func, upper_bound, lower_bound, value_type, rate)
        )
    ...
    def add_patch(
        ...
        if not isinstance(multipolygon, (Polygon, MultiPolygon)):
            raise TypeError(
                f'Wrong type "{type(multipolygon)}"' f" for multipolygon input."
            )
        ...
        if expansion_rate is not None:
            ...
            interiors = [inter for ply in multipolygon for inter in ply.interiors]
            ...
            for i, window in enumerate(iter_windows):
                ...
                try:
                    mask, _, _ = rasterio.mask.raster_geometry_mask(
                        self.src, multipolygon, all_touched=True, invert=True
                    )
    ...
    def add_contour(
        ...
        # pylint: disable=E1123, E1125
        self.add_feature(contours, expansion_rate, target_size, nprocs=nprocs)
    ...
    def add_channel(
        ...
        self.add_patch(channels, expansion_rate, target_size, nprocs)
        ...
        with self.modifying_raster(driver="GTiff") as dst:
            ...
            for i, window in enumerate(iter_windows):
                ...
                if utm_crs is not None:
                    ...
                    _logger.info(f"Transform creation took {time() - start2:f}")
                    start2 = time()
                    win_feature = [
                        ops.transform(transformer.transform, linestring)
                        for linestring in win_feature
                    ]
                    _logger.info(f"Transform apply took {time() - start2:f}")
    ...
    def get_xy_memcache(self, window, dst_crs):
        ...
        if tmpfile is None:
            ...
            transformer = Transformer.from_crs(self.src.crs, dst_crs, always_xy=True)
            # pylint: disable=R1732
            tmpfile = tempfile.NamedTemporaryFile()
            xy = self.get_xy(window)
            fp = np.memmap(tmpfile, dtype="float32", mode="w+", shape=xy.shape)
            fp[:] = np.vstack(transformer.transform(xy[:, 0], xy[:, 1])).T
    ...
    def add_subtidal_flow_limiter(
        self, hmin=None, hmax=None, upper_bound=None, lower_bound=None
    ):
        ...
        with self.modifying_raster() as dst:
            ...
            for i, window in enumerate(iter_windows):
                ...
                hfun_values = np.abs((1.0 / 3.0) * (topobathy / dh))
...
def transform_polygon(polygon: Polygon, src_crs: CRS = None, utm_crs: CRS = None):
    if utm_crs is not None:
        transformer = Transformer.from_crs(src_crs, utm_crs, always_xy=True)
