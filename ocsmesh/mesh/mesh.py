...
class Rings:
    ...
    def __call__(self):
        ...
        for poly in polys:
            data.append(
                {"geometry": poly.exterior, "bnd_id": bnd_id, "type": "exterior"}
            )
...
class Boundaries:
    ...
    def auto_generate(
        ...
        for poly in polys:
            ...
            ext_ring = np.array(
                [
                    (coo_to_idx[ext_ring_coo[e]], coo_to_idx[ext_ring_coo[e + 1]])
            ...
            if len(ocean_boundary) != 0:
                ...
                ocean_segs = (
                    [ocean_segs] if isinstance(ocean_segs, LineString) else ocean_segs
                )
                ocean_boundaries = [
                    [
            ...
            if len(land_boundary) != 0:
                ...
                land_segs = (
                    [land_segs] if isinstance(land_segs, LineString) else land_segs
                )
                land_boundaries = [
                    [
                        (coo_to_idx[seg.coords[e]], coo_to_idx[seg.coords[e + 1]])
        ...
        for poly in polys:
            ...
            for interior in interiors:
                ...
                int_ring = [
                    (coo_to_idx[int_ring_coo[e]], coo_to_idx[int_ring_coo[e + 1]])
...
class EuclideanMesh2D(EuclideanMesh):
    ...
    def interpolate(
        self, raster: Union[Raster, List[Raster]], method="spline", nprocs=None
    ):
    ...
    def get_multipolygon(self, zmin=None, zmax=None):
        ...
        clipped_mesh = utils.clip_mesh_by_vertex(
            self.msh_t, verts_in, can_use_other_verts=True
        )
    ...
    for window in raster.iter_windows(chunk_size=chunk_size, overlap=2):
        ...
        if np.any(zi.mask):
            m_interp = RegularGridInterpolator(
                (xi, np.flip(yi)), np.flipud(zi.mask).T.astype(bool), method=method
            )
        ...
        if method == "spline":
            ...
            f = RegularGridInterpolator(
                (xi, np.flip(yi)), np.flipud(zi).T, method=method
            )
